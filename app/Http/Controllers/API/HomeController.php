...
class HomeController extends Controller
{
    ...
    function list_of_shipment_and_delivery(Request $request){
        ...
        if($device_id)        
            $shipments = Shipment::select('shipment_id','status_dispatch','id_shipment_status','updated_at')->where('id_shipper', $member_id)->orWhere('id_device', $device_id)->get();
        else
            $shipments = Shipment::select('shipment_id','status_dispatch','id_shipment_status','updated_at')->where('id_shipper', $member_id)->get();
        $delivery = SlotList::select('slot_id','status_dispatch', 'sold_baggage_space', 'slot_price_kg', 'id_slot_status', 'updated_at')->where('id_member', $member_id)->get();

        // dd($member_id, SlotList::all());

        $outshipment = [];
        foreach ($shipments as $key => $shipment){
            if(!($shipment->id_shipment_status == 15 && $this->isADayAfter($shipment->updated_at)))
                array_push($outshipment, $shipment);
        }

        $outdelivery = [];
        foreach ($delivery as $key => $deliv){
            if(!($deliv->id_slot_status == 7 && $this->isADayAfter($deliv->updated_at)))
                array_push($outdelivery, $deliv);
        }
